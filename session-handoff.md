...
2026-07-19（**通用 RAG 写库 P1 与 feat-403 后台任务可靠性均已提交**）

## 当前状态

- 当前分支：`main`；`c553d6f` 为通用 RAG 批量写入提交，后一笔独立提交为 feat-403 后台任务队列，工作树干净。
...
- 通用 RAG Storage P1 已由 `c553d6f` 提交：默认 50 行/INSERT（可配置 1..500），同 document advisory lock，Dimension Guard/版本选择/replace 删除/全部批次统一事务。真实 pgvector 200 行 SQL 200→4，中位耗时 50.16ms→4.58ms（10.95x）。
- feat-403 已完成并提交：Brief/Campaign 的 `async_jobs` 支持按 kind 全局并发、SKIP LOCKED、worker lease/heartbeat、最多 3 次重试、过期/NULL lease 恢复和优雅退出。真实 PG 普通执行与 dead-worker 恢复均通过；完整 Nest `/health` 200；全仓 648 tests、typecheck/lint、API build 通过。
- ingestion EventEmitter/SSE 与 Agent SSE runner 本轮保持不变；后续若要统一持久化它们，必须分别设计进度事件重放、Agent 输入/step 恢复和幂等副作用，不能只复用 async_jobs payload。
