...
**数据覆盖**：Run-001～Run-008（8 次实验，3 个测试文档，共约 500 次 pipeline 调用）
...
| Run-007 | Bloomnote PRD | 6 | 4 | 2×2 因子（transform × reranker）| bge-base domain mismatch 发现 |
| Run-008 | Bloomnote PRD | 6 | 4 | **Reranker 横向对比** | score-only / bge-v2-m3 / pipeline / LLM-Qwen |
...
| ~~中文优化的 reranker 是否比 score-only 更好？~~ | **已回答（Run-008）**：bge-v2-m3 和 bge-base 失败模式相同；LLM reranker 是最优（hitRate=0.94）| — |
| LLM reranker 是否适合生产环境（延迟/成本）？| T04 每 query 约 44s，不可接受 | 测试 qwen-turbo 或批量化方案 |
| BCE reranker 是否优于 bge 系列？| 未测试 | 加入 `maidalun1020/bce-reranker-base_v1` 对比组 |
...
### Reranker 选择决策树（基于 Run-007/008）

```
中文产品文档场景
  ├── 实时生产（延迟 < 2s，成本敏感）
  │     → score-only，threshold=0.5
  │        理由：hitRate=0.89，超过 bge 系列所有版本
  │
  ├── 离线评估 / 质量优先（可接受 30-60s/query）
  │     → llm-relevance-rerank（qwen-plus），threshold=0.5
  │        理由：hitRate=0.94，目前已验证最高
  │
  └── Cross-encoder reranker（bge 系列）
        → ❌ 不推荐用于中文产品文档
           原因：domain mismatch（QA 训练任务 vs 产品文档查询）
           bge-base 和 bge-v2-m3 失败模式完全相同
```

**为什么 bge cross-encoder 在这个场景下失败？**

Cross-encoder 的 softmax 归一化导致分数极度集中（top-1: 0.8+，其余: <0.02）。
当 top-1 是错误 chunk 时，所有其他 chunk（包括正确的）都被压到 threshold 以下 → 全部 miss。
而余弦相似度分布平滑（0.4-0.7），多个相关 chunk 都能在 threshold=0.5 附近，更容易同时命中。

**Chunk 策略**（不受 reranker 影响，独立生效）：
- 文档章节短（< 512 字符）或查询以跨章节综合为主 → `recursive/512`
- 文档有明确章节结构且查询以单章节精确为主 → `markdown-heading/1024`
- 不确定查询分布 → `recursive/512`（跨章节覆盖更稳健）

> ⚠️ heading-context transform：Run-001 的 +0.11 发现已被 Run-007 证伪为边界效应，不再推荐。
