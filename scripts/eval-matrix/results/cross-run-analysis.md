...
| 中文优化的 reranker 是否比 score-only 更好？| Run-007 仅测试 bge-reranker-base，存在 domain mismatch | 测试 bge-reranker-v2-m3 或 BCE reranker |
...
> ⚠️ Run-007 发现：bge-reranker-base 在产品文档场景下存在 domain mismatch，hitRate 低于 score-only。
> 建议先验证 reranker 模型的质量（见下方"reranker 模型选择"），再决定是否启用。
...
**Reranker 模型选择**：
- bge-reranker-base：中英双语，但训练以 QA 任务为主，对产品文档类查询存在 domain mismatch
- 待验证：`BAAI/bge-reranker-v2-m3`（多语言增强版）、`maidalun1020/bce-reranker-base_v1`（中文优化）
- 在验证 reranker 质量之前，`score-only` 是更安全的默认选项
