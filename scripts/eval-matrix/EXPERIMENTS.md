...
## 实验 6：组合验证（summary-keywords + metadata-boost）

**目标**：验证实验 3 和实验 5 的收益是否正交叠加

| ID | transform | rerank | citationCoverage | confidence |
|---|---|---|---|---|
| T01 | none | score-only（基线） | 0.89 | 0.62 |
| **T02** | **summary-keywords** | score-only | **1.00** ✅ | 0.63 |
| T03 | none | metadata-boost | 0.94 | 0.75 |
| T04 | summary-keywords | metadata-boost | **0.72** ❌ | 0.79 |

**关键发现：两者组合产生干扰效应，citationCoverage 反而最低。**

根因分析：
- `summary-keywords` 在每个 chunk 的 `text` 字段末尾注入了「关键词: xxx 摘要: xxx」
- `metadata-boost` 读取 `m.text` 来做 jieba 分词关键词命中计数
- 注入的关键词后缀天然与 query 高度匹配 → boost 分数虚高 → 错误排序挤出真正相关的 chunk

**结论：两者不能同时开，必须二选一：**
- `summary-keywords`（citation=1.00）> `metadata-boost`（citation=0.94）
- **最终选定 `summary-keywords` 为默认 transform，rerank 保持 `score-only`**

> 潜在修复方向：metadata-boost 改为读取 `c.text`（原始文本，不含注入内容）而非 `m.text`，但需要改动 rerank 数据结构，留作后续优化。

---

## 综合最优配置（单文档场景，经实验 6 验证）
...
| **Transform** | **summary-keywords** | keywordCount=5, summaryMaxTokens=100, appendToChunk=true | citation=1.00，实验 6 验证最优 |
...
| Rerank | score-only | rerankTopN=5 | 与 summary-keywords 同时开时优于 metadata-boost |
...
**Q: 为什么 summary-keywords 和 metadata-boost 不能同时开？**

A: metadata-boost 通过读取 chunk 的 `text` 字段做关键词命中计数来加权排序。当 summary-keywords 已经把「关键词: xxx」注入到 text 末尾后，boost 计算时这些关键词天然与 query 高度匹配，导致加权分数虚高，错误地把某些不那么相关的 chunk 排到 top-5，挤掉了真正含有答案的 chunk。本质是两个优化目标的耦合冲突：summary-keywords 优化 embedding 召回，metadata-boost 优化排序，但后者的输入被前者污染了。修复方案是 metadata-boost 改用原始 text（不含注入内容），目前作为技术债留待后续。
