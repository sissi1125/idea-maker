...
## Agent 模式失败收尾与凭据诊断（2026-07-15）

### 根因
- 最近 Agent run 在首个 LLM 请求失败（`steps_used=0`、无 `agent_steps`），不是 ReAct/tool/RAG 故障。
- 使用 `.env.production` 的 `LLM_BASE_URL`、`LLM_MODEL`、`LLM_API_KEY` 直连 provider，确认返回 HTTP 401：令牌已过期或验证不正确；key 未输出或写入仓库。
- 前端收到 SSE `error` 后只在 Trace 内显示错误，没有通知对话页结束 `running`，因此用户看到一直“思考中”。
- `useAgentRun` 的辅助状态和步骤未按 `runId` 隔离，连续重试可能沿用上一轮 error/finalText/steps。

### 修复
- 后端识别常见 401/403 与中英文认证错误，统一返回脱敏、可操作的 `llm_auth` 错误；其他生产异常继续只暴露 event ID。
- AgentTracePanel 新增 `onError` 终态回调，对话页收到后切换 `phase=done` 并展示错误与经典模式回退。
- `useAgentRun` 将 cost/finish/error 绑定到所属 `runId`，并过滤旧 run 的步骤，保证连续重试状态隔离。

### 验证
- Agent runner 定向单测：16/16（新增认证错误分类与脱敏回归）。
- api/web typecheck + lint 全通过。
- 浏览器 E2E 连续运行两次：均从“思考中”切为“回复”，显示 `Agent · failed`、明确凭据提示和回退按钮；第二轮没有旧 prompt/步骤污染。
- 用户提供新智谱 key 后，已写入 Git 忽略的 `apps/api/.env` 并重启 API；直连 provider 返回 HTTP 200。
- 真实 Agent E2E 成功：`Agent · done`，依次记录 `generate_draft` tool call、tool result、reasoning 三条可观测步骤，返回最终标题与创作理由，成本约 `$0.0001`。

---

## 最新 main 回归阻断修复（2026-07-15）

### 回归结果与修复
- `AgentContextPanel.tsx` 在 effect 开头同步设置 loading/error，被 React 19 lint 规则阻断。改为按 `projectId + runId` request key 派生 loading/error；异步完成后才提交状态，保留同一上下文二次打开的缓存展示。
- `AgentRunnerService` 新增 `saveContextSnapshot()` 后，单测 repository mock 未同步，导致 15 个 Agent runner 测试失败。补齐 mock，并新增“调用 LLM 前保存真实 system prompt/messages”断言。
- `init.sh` 的 `$RECORDED_HEAD，` 在当前 locale 下把全角逗号解析进变量名，`set -u` 报未定义变量。改成 `${RECORDED_HEAD}` / `${CURRENT_HEAD}` 显式边界。

### 验证
- 定向 Agent runner：17/17。
- 全量：rag-core 239/239，API 365/365，共 604/604。
- API/Web typecheck、lint 全通过；Nest + Next 生产构建退出码 0。
- 浏览器 E2E：Agent 对话打开“查看上下文”后正常结束 loading，并展示真实 system prompt、input messages、产品知识、记忆、平台规则和工具列表；清除 3001 旧进程环境后，真实 GLM 再次完成 `generate_draft → tool result → reasoning → done`。
- feature 状态不变：相关 Agent 功能仍为 done，本次仅修复 main 回归门禁。

---

## 官网 Embedding / RAG 写库批处理 P0 优化（2026-07-19）

### 完成
- 官网正文chunk先展平，Embedding从逐条串行请求改为默认16条/批；严格按provider `index` 对齐，单条缺失或维度错误仅对应chunk降级NULL。
- 所有外部请求完成后才借PoolClient；rag_chunks从逐行INSERT改为默认50行参数化多值INSERT，并复用一次连接。批量向量写失败时整批NULL重试，保留稀疏检索正文。
- 新增 `WEBSITE_EMBEDDING_BATCH_SIZE`（上限64）与 `WEBSITE_RAG_INSERT_BATCH_SIZE`（上限200），非法配置回退安全默认值。

### 测试与前后对比
- 4个专项测试覆盖批次数、乱序index、无Key/维度错误降级、批量向量失败NULL重试；全仓typecheck/lint、API build、rag-core 239 + API 398 = 637 tests通过。
- 可重复基准 `pnpm --filter @harness/api bench:sources` 连跑两次：64 chunks、固定15ms provider往返，Embedding 64→4次请求，约15.5–16.2x。
- 真实本地pgvector、200行、5轮中位数：SQL 200→4条，约17.2–18.0x。该结果隔离网络/SQL往返，不代表含400ms抓取限速和图片下载的端到端导入也提升同等倍数。
- API/schema无变化，不更新 `docs/API_CONTRACTS.md`。

---

## PostgreSQL 连接池 P0 优化（2026-07-19）

### 完成
- `DbService` 从每次 `new Client/connect/end` 改为进程级单例 `pg.Pool`：默认 `max=10`、idle 30s、建连超时 5s，支持环境变量覆盖；业务请求 `finally release()`，Nest 收到退出信号后统一 `pool.end()`。
- 并发首请求通过共享 Promise 只执行一次 DDL；初始化失败可在下一请求重试。
- 新增最小 `DbClient` 契约，生产业务只暴露 `query()`，不允许越层调用 connect/end/release；旧 Playground 动态 connectionString 仍使用隔离的专用 Client。
- ingestion storage 纳入 Pool；Agent 后台路径改用按查询借还的 `queryClient()`，等待 LLM/embedding 时不占连接池槽位。

### 验证
- API typecheck、lint、生产 build 通过；全量 API 40 files / 394 tests 通过，其中连接池新增 6 个生命周期与并发初始化测试。
- 真实 PostgreSQL：`DB_POOL_MAX=2` 下并发 5 个 100ms 查询耗时约 470ms，观测为 1 active + 1 idle，证明最多 2 条连接并产生排队；查询完成连接可复用，销毁时 pool 正常关闭。
- API/schema 无变化，不需要更新 `docs/API_CONTRACTS.md`。`session-handoff.md` 当前存在用户未解决的 merge conflict，本轮未覆盖。

---

## 生产 Named Tunnel 故障恢复（2026-07-18）

### 根因与恢复
- 公网 `/health` 返回 Cloudflare `530 / 1033`；ECS、Docker、Postgres 和 API 均正常，`cloudflared` 于 7 月 12 日因 Docker DNS 异常后收到 SIGTERM，以 exit 0 停止且未再拉起。
- 在 ECS 以 compose 恢复 `cloudflared`，4 条 tunnel connection 注册成功；公网 `/health` 恢复 HTTP 200。
- 部署工作流在 API healthy 后使用 `up -d --no-deps cloudflared` 幂等确保隧道在线，并让 API health 超时真正阻断部署。

### 验证与风险
- 生产 API、Postgres healthy；公网 `https://api.retreevo.online/health` 返回 200。
- ECS 根盘使用率 91%（40G 仅余 3.4G），需另行审计 Docker/日志占用后再清理，当前未删除任何数据。

---

## feat-401 Agent Grounding 阶段 0/1（2026-07-15）

### 完成
- 统一 `AgentGroundingContext`：只聚合整体 confirmed Brief 的 confirmed 字段、approved Claims、evidence provenance 和平台规则；移除 Agent 对 `auto_generations` 产品摘要的事实依赖。
- raw RAG chunk 原文不再注入模型，防止同段未确认事实绕过 Brief；字段与 Claim 保留 chunk IDs 供追溯。
- `generate_draft` / `refine_draft` / `critic_review` 共用服务端 Grounding；无 Brief 不调用 nested LLM，citation、无依据价格规格、平台规则失败均 fail closed。
- 仅禁词失败时允许确定性删除并重新校验，删除项写入 tool result；其他平台失败只在事实已通过时暴露 `candidateDraft` 给 refine。
- critic 使用 JSON mode 兼容 GLM；确定性门禁优先于模型分数；passed 后清空建议。AgentRunner 最终交付最近一次 critic passed 的精确 draft，outer 转述不能删除引用或改事实。
- 新增 Grounding 专用 golden `gold-product-brief-grounding-006` 与 5 道面试题；规格同步到 `docs/PRODUCT_BRIEF_ITERATION_PLAN.md`。

### 验证
- 全仓测试 626/626（rag-core 239 + API 387）；全仓 typecheck/lint、Nest/Next 生产构建和 `./init.sh` 通过。
- 真实 Bloomnote eval：`eval_run_id=a6b6b5e3-a8c2-4b73-901f-b7560bbf5b8c`，1/1 passed；faithfulness=4、completeness=5、style=4、overall=4.333。
- 回归过程捕获并修复：完整 citation 标签解析不兼容、GLM critic tool-call object 不兼容、outer 最终转述删除引用、raw chunk 泄漏未确认事实、critic passed 后仍建议 refine。

### 评测说明
- 旧 5 条通用 golden 是护肤/节日/环保包装等跨产品样例，对 Bloomnote 正确拒答会判低分，不能作为项目级 Grounding 质量基线。后续 eval 必须按 Product Brief 选择对齐 case，或建立“应拒答”评分类型。
# 2026-07-18（feat-402：Klaviyo 风格 UI 产品化，进行中）

## 已完成

- 建立墨黑/纯白/暖灰/珊瑚色设计 token，统一按钮、表单、标签、卡片、表格、焦点与响应式状态；移除 `next/font` 远程下载，生产构建不再依赖 Google Fonts。
- 重构 Login：真实表单 + 产品信息/来源/核查预览双栏，删除未接通 OAuth 占位按钮。
- 导航收敛为：项目总览 / 产品资料 / 产品信息 / 内容创作 / AI 对话 / 内容资产 / 项目设置。
- 新增项目总览（现有接口派生下一步）和内容资产聚合页；新建项目默认进入总览。
- 产品资料去 emoji，完成后进入产品信息；内容创作改成任务/平台/受众/场景/CTA/避免项/卖点的结构化表单，小红书仅为当前默认重点平台。
- AI 对话保留一级入口并完成视觉迁移；移动端改为顶部项目栏 + 展开导航。
- E2E 修复真实 bug：后端 ingestion 终态为 `succeeded`，前端只判断 `completed` 导致成功后无限轮询；现兼容两个终态。

## 验证

- `pnpm -r typecheck`：通过。
- `pnpm -r lint`：通过，0 warning。
- `pnpm -r test`：rag-core 239 + API 387，共 626 tests 通过。
- API build 与 Web production build：通过（Web 需沙箱外运行，Turbopack 会绑定本地端口）。
- PostgreSQL healthy，pgvector 0.8.2；NestJS API `http://localhost:3001` 全模块启动，health 200。
- 浏览器真实链路：注册 -> 创建项目 -> 默认进入总览 -> 上传中文 Markdown -> ingestion ready -> 产品信息 -> 多平台内容创作 -> AI 对话 -> 内容资产。
- 数据库断言：测试用户 1、项目 1、document 1、rag_chunk 1、ready document 1。
- 桌面截图通过；375px `scrollWidth=375`，移动导航完整，无横向溢出；浏览器 console 0 errors。

## 未完成

- 当前 worktree 没有 LLM/Embedding 凭据，未执行真实外部模型的 Brief 提取、Campaign 生成和 Agent 完整链路，不能标记为通过。
- 生产 API `https://api.retreevo.online/health` 当前经 Cloudflare 返回 530；本 feature 未写生产数据，也未执行部署修复。

## 2026-07-19 第二轮视觉收敛与内容状态机

- 删除珊瑚红主色，改为石墨、冷白、冷灰和低饱和蓝灰；成功状态不再使用亮绿色标签，空状态统一取消斜体。
- 对标站实测正文/组件字体为 Instrument Sans；引入自托管 `Instrument Sans Variable`，中文回退苹方/微软雅黑，生产构建不依赖远程字体。
- 产品信息由路由统一渲染 PageHeader 一级标题；版本与档案状态独立为元信息。字段从卡片墙改为分组审计行，常见字段使用中文名称，视觉资产压缩为同层区段。
- 新增纯前端内容工作流投影：drafting / ready / generating / reviewing / accepted / failed，界面展示设置任务、生成内容、审核选择、采纳保存四步；后端模型和 schema 未改。
- 验证：`./init.sh`、Web production build、全仓 626 tests 通过；本地 Web/API health 均为 200。浏览器实际完成注册、创建项目、产品信息、内容创作；统一 h1、字体、无旧绿色/斜体，桌面和 375px 均无横向溢出或控件重叠。

## 2026-07-19 第三轮蓝色体系、Guide 与表单统一

- 参考 Hyperbound 实际页面，将背景调整为浅灰白，侧栏使用 `#F7FBFF`，以 `#2563EB / #ECF2FF` 作为主操作和选中态点缀；不采用设计检索建议的橙色 CTA 或玻璃效果。
- 全局圆角提升到 8-14px；按钮、卡片、导航和输入补充稳定的蓝色 hover、focus 与轻量阴影。
- 新增共享 `SelectField`，统一箭头、圆角、hover/focus；产品工作区 Campaign、Product Brief、视觉资产、海报、设置和记忆表单已迁移，开发 Playground 不在本次产品 scope。
- 项目总览新增解释型 ProjectGuide：四步各带一句说明，并与“建议下一步”合并为一个整体；内容创作移除重复的项目级 ProductSteps，只保留任务状态轨道。
- 字体从 Instrument Sans + 苹方回退切换为自托管 `Noto Sans SC Variable`，不依赖系统苹方或远程字体。
- 验证：全仓 typecheck/lint、626 tests、Web production build、`./init.sh` 通过；本地 API/Web 200。浏览器验证 Guide、共享 Select（appearance none + 自定义箭头）、内容创作无重复步骤；桌面及 375px 无溢出。

## 2026-07-19 产品信息页双区重构

- 移除产品信息页顶部 ProductSteps；页面正文拆分为“产品信息”和“视觉资产”两部分。
- 产品信息变为固定高度的纵向滚动 block，增加 sticky 锚点导航：产品身份 / 产品事实 / 用户与场景 / 定位与差异化 / 卖点建议。卖点建议直接复用 Claim Map，可派生、批准和拒绝，不建立第二份事实。
- 视觉资产改为图片卡片画廊，支持四种业务标签：Logo / 主图 / 氛围素材 / 功能截图，并可通过 Select 关联当前项目卖点。官网新抓取主图使用 `hero_image`；旧枚举继续兼容。
- 最小后端扩展：`visual_assets.claim_id` 可空外键，新增 `PATCH /projects/:projectId/assets/:assetId/tags`；关联卖点强制校验同项目。RAG、Brief、Campaign 和生成服务未改。
- E2E：浏览器上传 2618×2576 PNG 成功；真实 PATCH 保存 `atmosphere`，创建并关联测试卖点后 GET 回读匹配，页面刷新后两个 Select 正确回显。桌面产品信息 scrollHeight 679/clientHeight 488，375px 页面无横向溢出。
- 验证：全仓 626 tests、typecheck/lint、API/Web build、`./init.sh` 通过；本地 API/Web 200。

## 2026-07-19 产品档案三级工作区与可维护资产

- 一级导航「产品资料 / 产品信息」更名为「资料库 / 产品档案」，页面 H1 与总览入口同步统一。
- 产品档案使用三个同级 Tab：产品信息 / 产品卖点 / 视觉资产。产品信息内部只保留产品身份、产品事实、用户与场景、定位与差异化四个二级锚点，标题字号遵循 H1 / 一级 Tab / 区域 H2 / 锚点 H3 层级。
- 产品卖点继续复用 Claim Map：平台从确认信息生成默认内容，用户可新增、编辑、删除；编辑已批准卖点后自动回到待审核，避免静默替换已批准表达。
- 视觉资产上传入口成为网格末尾的统一卡片：无需预选类型，点击整卡直接选择图片，支持 `multiple` 批量上传，默认先标为功能截图，再由用户修改图片类型与对应卖点。补充图片删除并同步清理存储文件。
- API 最小补充 `PATCH/DELETE claims/:claimId` 与 `DELETE assets/:assetId`；RAG、Brief 提取、Campaign、Agent 与生成服务未改。
- 验证：全仓 typecheck/lint、626 tests、API/Web production build、`./init.sh` 全通过；本地 Web/API health 为 307（登录跳转）/200。浏览器真实 E2E 完成三级 Tab、卖点新增→编辑→删除、上传卡片 file chooser `multiple=true`、图片上传→删除；375px `innerWidth=scrollWidth=375`，Tab/资产卡片均为 343px，无横向溢出。

## 2026-07-19 统一确认弹窗与横向资产导航

- 新增共享 `ConfirmDialog`：统一遮罩、危险操作图标、标题说明、取消/确认按钮、Esc/点击遮罩关闭、打开时锁定页面滚动，并使用 `alertdialog`/`aria-modal` 语义。
- 项目删除、产品卖点删除、视觉资产删除三处浏览器原生 `confirm` 已全部替换；前端代码不再包含原生 `alert/confirm` 调用。
- 修复产品档案视觉层级：一级分类改成稳定的横向下划线 Tab；版本、状态、整理与确认工具栏只在“产品信息”中显示；移动端保持横向布局，不再堆成三张纵向卡片。
- 内容资产从入口卡片墙改为横向 Tab：笔记库 / 内容包 / 海报 / 生成记录 / 评估报告，默认选中笔记库，下方仅展示当前资产类型与进入操作。
- 验证：全仓 typecheck/lint、626 tests、API/Web production build 通过；本地 API 使用真实 PostgreSQL/JWT 配置重启后 health 200。浏览器验证自定义弹窗取消不删除数据、无原生 JS dialog、内容资产默认笔记库；375px 两页 `innerWidth=scrollWidth=375`，Tab、工具栏、面板无横向页面溢出。

## 2026-07-19 内容资产真实 Tab 聚合修正

- 修正上一版把横向 Tab 做成二次跳转入口的问题：内容资产现在直接挂载笔记库、内容包、海报、生成记录、评估报告的现有业务视图，切换期间 URL 保持 `/assets`，不再出现“打开某页面”的中转卡片。
- Tab 改成固定 104-112px 宽的分段控件，文字与图标不会互相挤压；容器不足时仅 Tab 自身横向滚动。
- 复用独立笔记库/生成记录路由时，通过聚合容器样式移除第二层页面留白和滚动，不复制 API 请求、编辑、反馈等业务逻辑；原独立路由仍可继续访问。
- 验证：全仓 typecheck/lint、626 tests、API/Web production build 通过。浏览器逐项点击 5 个 Tab：URL 全程不变，内容包表单、海报工作台、生成历史、Eval 报告均在当前面板真实渲染；默认笔记库显示真实列表/空态。375px 下页面 `scrollWidth=375`，Tab 容器 333px、内部滚动宽 418px，内容面板宽 333px。

## 2026-07-19 本地数据库启动修复

- 定位“数据库未配置”并非业务代码回归，而是同一 worktree 残留多个 `ts-node-dev` 父进程，文件热更新后轮流争抢 3001，缺少环境变量的旧实例最终占用端口。
- 清理本 worktree 的重复 API 进程，仅保留一个实例；新增 Git 忽略的 `apps/api/.env`，固定本地 `DATABASE_URL`、开发 JWT 与 Web CORS，使后续热重启持续加载正确配置。
- 验证：3001 仅一个监听进程；`GET /health` 返回 200；真实 `POST /auth/login` 返回 200 并识别 `ui-redesign-20260719@test.local`。

## 2026-07-19 交互稳定性、任务维护与来源优先级

- 内容资产改为固定标题/Tab + 独立内容滚动区；切换不同高度的 Tab 时导航不再抖动，滚动条预留稳定空间。
- 资料库官网来源增加稳定 loading；已有官网直接显示当前 URL/页面，点击编辑才出现回填输入框。更换官网成功后清理旧来源、页面和对应 RAG chunks，失败时保留旧来源。
- 内容任务列表返回并展示本次允许卖点；新增自定义确认弹窗删除任务，候选内容和评估记录同步清理。
- 经典生成与 Agent 对话均向共享 Markdown 传递原始 evidence；`[evidence-N]` hover/click 展示 chunk 原文和来源引用。
- Brief 字段继续使用既有 source/confidence；Claim 新增 `user|platform` 来源，视觉资产新增 `user|website|document|platform` 来源。用户维护 Claim 优先进入生成上下文，自动海报资产按用户上传、官网抓取、其他来源排序。
- API 契约补充 Campaign DELETE、官网 `replaceExisting`、Claim/Asset `origin` 及 Agent context evidence。

### 验证

- 运行态 PostgreSQL 已确认 `claims.origin` 与 `visual_assets.origin` schema 生效。
- 全仓 626 tests、API/Web production build 已通过；最终顺序 typecheck、lint 与 `git diff --check` 通过。
- 浏览器 E2E：内容资产桌面与 375px 切 Tab 前后导航坐标不变，只有内容区滚动且无横向溢出；官网 fixture 首屏直接展示并可编辑/取消；用户卖点批准后创建任务，任务展示卖点标签并成功删除；产品档案卖点与图片均展示“用户维护/优先级高”来源。
- 阻塞：项目设置仍为 `key_missing`，未执行 Brief extract、Campaign generate 与 Agent generate/critic/refine 的真实外部模型链路。拿到 LLM Key 后必须补跑，`feat-402` 继续保持 `in-progress`。

## 2026-07-19 “确认全部信息”状态闭环修复

- 根因：原接口只把 Product Brief 容器改为 `confirmed`，没有更新 `candidate/stale` 子字段；前端也始终渲染“确认全部信息”按钮。
- 后端将批量确认改为单事务：逐条复用 `confirmField` 更新子字段并写 revision，再确认 Brief；任一步失败整体回滚。
- 前端按钮可用条件改为三个必填字段已有候选/待复核/确认值；全部完成后按钮替换为只读“全部信息已确认”状态。
- 新增单测验证 candidate/stale 均被确认且逐条写 revision；API 契约同步批量确认语义。

### 验证

- 浏览器 + API + PostgreSQL 真链路：准备 3 个必填 candidate + 1 个可选 stale，点击一次后 4/4 子字段为 confirmed、4 条 revision、Brief 为 confirmed v2；逐项确认按钮和右上角批量按钮均消失，显示“全部信息已确认”。
- 隔离测试项目、账号和字段 fixture 已全部删除。

## 2026-07-19 feat-402 合并与生产部署

- `8ff2318` 已从 `b44c611` 快进推送到 `origin/main`；Vercel Production 部署成功，CI 与后端镜像/ECS 部署 workflow 均成功。
- 生产 API 仍返回 Cloudflare `530 / 1033`。根因是旧 workflow 只重启并检查 API，明确不处理已停止的 `cloudflared`，因此会出现部署假成功。
- 部署 workflow 增加硬门禁：API 未在超时内 healthy 则失败；API healthy 后使用 `up -d --no-deps cloudflared` 幂等恢复 Named Tunnel，并校验容器处于 running。
- 修复提交 `eb1f57f` 推送后，CI 与 Deploy Backend 均成功；Named Tunnel 恢复，生产 API `/health` 返回 200。Vercel 生产别名返回 200 并展示新版 Login。

## 2026-07-19 PostgreSQL 连接池与官网 RAG 批处理 P0

- `bdc8dd0` 将生产 `DbService` 改为进程级 `pg.Pool`；业务查询按需借还连接，事务仍固定在同一 `PoolClient`，Playground 动态 `connectionString` 保持短生命周期专用 Client。
- `2c3f9a0` 将官网 Embedding 改为默认 16 条/请求、官网 `rag_chunks` 改为默认 50 行/INSERT，并保留批次失败的逐条降级路径。
- 基准：64 chunks 的 Embedding 请求数 64 -> 4，约 15.5-16.2x；真实 pgvector 200 行写入 SQL 数 200 -> 4，中位耗时约 17.2-18.0x。
- 验证：637 tests、全仓 typecheck/lint、API build 和 `git diff --check` 通过。
- 下一步按独立改动推进：先完成通用文档 RAG chunk 批量写入和事务原子性，再实现后台任务并发限制、租约与重启恢复。

## 2026-07-19 通用 RAG Storage 批量写入 P1

- `packages/rag-core` 的 chunk 写入由逐条 INSERT 改为参数化多值 INSERT，`batchSize` 默认 50、范围 1..500；生产 ingestion 与 Playground 三种 storage method 均接入。
- Dimension Guard、版本选择、replace 删除与全部 INSERT 批次使用同一事务；失败整体 ROLLBACK。同 document 事务先获取 advisory lock，避免并发 `new-version` 选到相同版本，不同文档仍可并行。
- 新增 120 chunks -> 3 INSERT、参数顺序/enhancedText fallback、upsert/error SQL、事务顺序和第二批失败回滚测试；补 API 契约、基准脚本与 5 道面试题。
- 真实本地 pgvector 基准（同连接/同临时表、200 行、7 轮中位数）：SQL 200 -> 4，50.16ms -> 4.58ms，约 10.95x。该结果只代表 SQL 往返段，不等于完整 ingestion 端到端加速。
- 验证：rag-core 242 + API 398 = 全仓 640 tests；全仓 typecheck/lint、API build、`git diff --check` 通过。
- 下一步：以独立 feature/commit 实现后台任务并发限制、租约、心跳、过期任务回收和启动恢复。

## 2026-07-19 feat-403 PostgreSQL 后台任务队列可靠性

- Product Brief 提取和 Campaign 生成从 `runInBackground` 进程内闭包迁移到 `async_jobs` PostgreSQL 队列；启动端点仍立即返回 jobId，前端轮询状态协议不变。
- 任务持久化最小 payload，业务模块按 kind 注册恢复 handler；不复制 API key/prompt，执行时重新读取项目数据库配置。
- worker 在事务中用 kind advisory lock 保护跨实例并发计数，再用 `FOR UPDATE SKIP LOCKED` 抢占；2C2G 默认 Brief/Campaign 各 1 并发，可通过环境变量独立调整。
- 默认 lease 60s、heartbeat 20s；失败最多 3 次并指数退避。启动/轮询回收过期及历史 NULL lease，优雅退出停止 claim、等待 10s 后释放仍未完成的任务。
- 8 个队列单测覆盖 payload、claim、单进程/跨实例并发、heartbeat、retry、shutdown 和旧表迁移顺序。真实 PostgreSQL 验证普通任务 attempt=1 成功，模拟 dead worker 后过期任务 attempt=2 恢复成功；测试行均已清理。
- 完整 Nest 应用在真实 schema 上启动，两个 handler 注册成功，`GET /health` 200。全仓 rag-core 242 + API 406 = 648 tests，typecheck/lint、API build、harness 和 `git diff --check` 通过。
- 范围边界：本 feature 只升级 `async_jobs`（Brief/Campaign）；ingestion 的 EventEmitter/SSE 和 Agent 的实时 SSE runner 保持原协议与实现，未混入可轮询业务队列。
