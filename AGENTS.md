...
- **阶段感知**：当前阶段以 `feature_list.json` 顶部的 `phase` 字段为准。各阶段范围参考 `docs/PRODUCT.md` 和 `docs/ROADMAP_PHASE2_PLUS.md`：
  - 阶段 1（已完成）：RAG Playground 可调试闭环。
  - 阶段 2（收尾）：完成 feat-006 / feat-008（RAG 质量评估与 Eval Matrix）。
  - 阶段 3：Agent 自动化层（Pipeline Agent + Content Agent）—— feat-010 / feat-011 系列。
  - 阶段 4：Marketing Studio UX —— feat-012 ~ feat-015。
  - 阶段 5：工程化（Auth / 多租户 / BYOK / 部署）—— feat-016 ~ feat-020。
