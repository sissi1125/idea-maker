...
- **阶段感知**：当前阶段以 `feature_list.json` 顶部的 `phase` 字段为准。各阶段范围参考 `docs/PRODUCT.md` 和 `docs/ROADMAP_PHASE2_PLUS.md`：
  ...
  - 阶段 2.5（架构重构）：feat-100 ~ feat-103（pnpm monorepo + NestJS 前后端分离）。
...
- **双轨并行**（详见 `docs/ROADMAP_PHASE2_PLUS.md#双轨并行执行模型`）：
  - **轨道 A 主流程**：架构重构 + Agent + Studio + 工程化（feat-100~103, feat-010~013 系列），分支 `claude/refactor-monorepo` 或类似。
  ...
  - **Wave 2 冻结窗口**：主流程做 Wave 2（feat-101，抽 packages/rag-core）期间约定 1-2 周，**实验流仅调参不动算法核心代码**。冻结开始/结束必须写入 `session-handoff.md`。
  - **rebase 节奏**：主流程每个 Wave 开始前 rebase main，拉取实验流合入的最新算法。
  - **编号约定**：实验流用 feat-006.x / feat-008.x / feat-009.x；主流程用 feat-010~013 与 feat-100+。禁止混用。
